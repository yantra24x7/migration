...
export class CycleTimeService {
  ...
  all_time_chart(register):Observable<any>{
    return this.http.get('all_cycle_time_chart?machine_id=' + register.machine_id + '&&shift_id=' + register.shift_id + '&&tenant_id=' + register.tenant_id + '&&date=' + register.date)
  }
  // Third Api
  shift_machine_status(register):Observable<any>{
    return this.http.get('hour_machine_status_chart?machine_id=' + register.machine_id + '&&shift_id=' + register.shift_id + '&&tenant_id=' +register.tenant_id + '&&date=' + register.date)
  }
...
hour_machine_utilization(register):Observable<any>{
  return this.http.get('hour_machine_utliz_chart?machine_id=' + register.machine_id + '&&shift_id=' + register.shift_id + '&&tenant_id=' + register.tenant_id + '&&date=' + register.date )
}
